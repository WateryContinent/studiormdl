...
int CJointedModel::ProcessSingleBody()
{
	...
	if ( !convexOut.Count() || !m_allowConcave )
	{
		...
		CPhysConvex *pConvex = ( vertsThisConvex.Count() > 3 )
			? physcollision->ConvexFromVerts( vertsThisConvex.Base(), vertsThisConvex.Count() )
			: NULL;
		if ( pConvex )
		{
			// Got something valid, attach this convex data to the root model
		...
		{
			MdlWarning( "Collision mesh '%s' is degenerate (flat/too few verts) - .phy skipped\n",
				m_pModel ? m_pModel->filename : "unknown" );
			return 0;
		}
	}
...
int DoCollisionModel( bool separateJoints )
{
	...
	// Load vphysics FIRST (original ordering) so that nothing is modified if it's absent.
	// If it fails, still consume the optional { ... } block so the QC parser doesn't choke.
	...
	if ( !physicsFactory )
	{
		...
		MdlWarning( "vphysics.dll not found - collision model will be skipped\n" );
		return 0;
	}
...
//	g_pPhysics2 = (IPhysics2*)physicsFactory(VPHYSICS2_INTERFACE_VERSION, NULL);
	...
	else
	{
		...
		if ( !pmodel )
		{
			// Still consume the optional { ... } block so the QC parser doesn't choke.
			if ( GetToken(true) )
			{
				if ( !strcmp( token, "{" ) )
					ParseCollisionCommands( g_JointedModel );
