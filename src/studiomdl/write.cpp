...
//	Physics2Collision_Write();
	...
	// Write RMDL v10 + VG directly from the in-memory MDL data.
	// Intermediate .mdl/.vvd/.vtx files are deleted afterwards;
	// the only outputs left on disk are .rmdl and .vg.
	WriteRMDLFiles( phdr, filename );
}
